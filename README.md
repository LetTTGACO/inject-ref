...
这个仓库不发布到插件市场。普通使用推荐直接打开 [GitHub 最新 Release](https://github.com/LetTTGACO/inject-ref/releases/latest)，在 Assets 里下载对应安装包。

VS Code、Cursor、Trae 等基于 VS Code 的 IDE：下载 `inject-ref-vscode-*.vsix`，然后在 Extensions 面板中选择 `Install from VSIX...` 安装。

如果使用 VS Code 命令行，也可以执行：

```bash
code --install-extension inject-ref-vscode-*.vsix
```

JetBrains 系列 IDE：下载 `inject-ref-jetbrains-*.zip`，打开 Settings -> Plugins，点击齿轮菜单，选择 `Install Plugin from Disk...`，再选择下载的 zip 包。

## 从源码编译安装

如果想自己改代码或重新打包，可以从源码编译本地安装包。
